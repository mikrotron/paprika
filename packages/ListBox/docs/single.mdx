...
`renderTrigger` allows for a different render label for the trigger.
...
To group options together, simply parent all `<ListBox.Option/>`
with `<ListBox.Group>`. Add `groupId=""` and
`label=""` prop to `<ListBox.Group>` to create the name of the group and its id.
